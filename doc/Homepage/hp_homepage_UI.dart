...
import '../../../onboarding_v2/presentation/widgets/add_extra_habits_sheet.dart';
import '../../../onboarding_v2/domain/services/habit_configuration_service.dart';
...
class _HomePageState extends ConsumerState<HomePage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Tutorial and goal wizard systems removed in v7.0 - new users go through onboarding_v2 instead
  }
    ...
    if (choice == 'template') {
      ...
      final selectedTemplateIds = await showModalBottomSheet<Set<String>>(
        ...
        builder: (context) => const AddExtraHabitsSheet(
         programCoreHabitIds: [], currentExtraHabitIds: {}, // Empty - user can pick any template
        ),
      );

      if (selectedTemplateIds == null || selectedTemplateIds.isEmpty || !mounted) {
        return;
      }
      ...
      if (mounted && successCount > 0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Added $successCount habit${successCount > 1 ? 's' : ''} successfully!')),
        );
      }
    } else if (choice == 'custom') {
      // Navigate to custom form
      Modular.to.pushNamed('/habits/new');
    }
  }
  ...
  Widget _buildHabitsList(AsyncValue<List<Habit>> habitsAsync, TimeOfDayEnum? timeFilter) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return EnhancedHabitCard(
                ...
                linkedGoals: const [],
                onTap: () => Modular.to.pushNamed('/habits/detail/${habit.id}'),
                onCompletionChanged: (completed) => _handleHabitCompletion(habit, completed),
                onEdit: () => Modular.to.pushNamed('/habits/edit', arguments: habit),
          ...
          builder: (context, goalsSnapshot) {
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final linkedGoals = goals.where((goal) => goal.habitIds.contains(habit.id)).toList();
  ...
  Widget _buildAIEmptyState(TimeOfDayEnum? timeFilter) {
    ...
    return AIEmptyState(
      ...
      additionalContent: timeFilter != null
          ? _buildTimeSpecificTips(timeFilter)
          : null,
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          ...timeSpecificTips.map((tip) => Padding(
  ...
  void _handleHabitCompletion(Habit habit, bool completed) {
    ...
    ref.read(habitCompletionProvider).toggleHabitCompletion(habit.id, selectedDate, ref);
  }
