...
/* EXAMPLE HOW I WANT THE ONBOARDING PAGE TO BE IMPLEMENTED AS UI ONLY - DO NOT DELETE THIS COMMENT 
...
class _OnboardingPageState extends ConsumerState<OnboardingPage> {
  ...
  void _handleOnboardingComplete() {
    ...
    if (currentUser != null) {
      _completeOnboardingForExistingUser(currentUser.uid);
    } else {
  ...
  Future<void> _completeOnboardingForExistingUser(String uid) async {
    try {
      final userRepository = UserRepository(FirebaseFirestore.instance);
      final userProfile = await userRepository.getUserProfile(uid);
      ...
      // Don't complete onboarding - let user go through onboarding_v2
      Modular.to.navigate('/');
    } catch (e) {
      if (mounted) {
...
class CurvedBottomPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(CurvedBottomPainter oldDelegate) => color != oldDelegate.color;
} */