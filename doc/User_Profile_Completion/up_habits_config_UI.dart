...
import '../../../../core/constants/program_templates.dart';
...
/// Reads programId from state machine
...
class _HabitConfigurationPageState
  ...
  List<String>? _customHabitIds; // For custom program
  bool _isLoading = false;
  bool _isLoadingCustomHabits = false;
  late AnimationController _pulseController;
  late Animation<double> _pulseAnimation;

  // Get programId from state machine
  ...
  void initState() {
    ...
    // Load custom habits if this is a custom program
  ...
  /// Get the index of the first unlocked (unconfigured) habit
  int get _firstUnlockedIndex {
    ...
    if (_programId == 'custom') {
      ...
      return habitIds.length; // All configured
    }
    ...
    return program.coreHabitIds.length; // All configured
  }

  /// Check if a habit is unlocked (can be configured)
  /// Extra habits (index 999) are always unlocked
  ...
  Future<void> _configureHabit(String habitId) async {
    final habitTemplate = HabitTemplates.all.firstWhere(
      (h) => h.id == habitId,
      orElse: () => HabitTemplates.all.first,
    );
  ...
  Future<void> _addExtraHabits() async {
    // Get core habit IDs based on program type
  ...
  Future<void> _completeConfiguration() async {
    ...
    try {
      ...
      // Create Habit models for all habits
      final habits = <Habit>[];
      for (final habitId in _allHabitIds) {
        ...
        final habit = Habit(
          ...
          description: '',
          soundId: 'default',
          repeatConfig: RepeatConfig.daily(), // Default to daily
      ...
      // Update state machine with configured habits
      // The state machine will handle saving to Firestore in final setup
      if (mounted) {
        ref
  ...
  Widget build(BuildContext context) {
    ...
    final programName = _programId == 'custom'
        ? 'My Custom Program'
        : program!.name;
    final programColor = _programId == 'custom'
        ? theme.primaryColor
        : program!.color;
          ...
          children: [
              ...
              child: Column(
                children: [
                  ...
                  LinearProgressIndicator(
                    ...
                    valueColor: const AlwaysStoppedAnimation(Color(0xFFFF6B35)), // Orange
                  ),
                ],
              ),
            ),
                ...
                children: [
                  ...
                  if (_programId == 'custom')
                    ...(_customHabitIds ?? []).asMap().entries.map((entry) {
                      ...
                      return _buildHabitCard(theme, null, habitId, false, index);
                    })
                  else
                    // Handle preset program
                    ...program!.coreHabitIds.asMap().entries.map((entry) {
                      final index = entry.key;
                      final habitId = entry.value;
                      return _buildHabitCard(theme, program, habitId, false, index);
                    }),
                  ...
                  if (_extraHabitIds.isNotEmpty) ...[
                    ...
                    ..._extraHabitIds.map((habitId) {
                      // Extra habits are always unlocked, so pass a high index
            ...
            Container(
              ...
              child: SizedBox(
                ...
                child: FilledButton(
                  onPressed: _isLoading || !_allConfigured
                      ? null
  ...
  Widget _buildHabitCard(
    ...
    // Make card semi-transparent if locked
            ...
            return Container(
              ...
              child: InkWell(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: isConfigured
                        ? const Color(0xFFFF6B35).withValues(alpha: 0.1) // Orange
                  ...
                  child: Row(
                    children: [
                      ...
                      // Remove button for extra habits
                      if (isExtra) ...[
